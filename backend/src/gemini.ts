...
async function callGeminiAPI(
  ...
  const hasImage = parts.some((p) => "inline_data" in p);

  console.log(`[Gemini] Calling API (parts: ${parts.length}, image: ${hasImage})`);
  ...
  if (!text) {
    throw new Error(`Gemini API returned no text: ${JSON.stringify(json).slice(0, 200)}`);
  }
...
async function fetchImageAsBase64(url: string): Promise<{ data: string; mimeType: string } | null> {
  ...
  console.log(`[Gemini] Fetching image: ${cleanUrl}`);

  try {
    const proc = Bun.spawn(["curl", "-sL", "--max-time", "15", "-o", "-", cleanUrl], {
      stdout: "pipe",
      stderr: "pipe",
    });
    ...
    if (proc.exitCode !== 0 || output.byteLength < 1000) {
      console.error(`[Gemini] Image fetch failed for ${cleanUrl} (exit: ${proc.exitCode}, size: ${output.byteLength})`);
      return null;
    }

    console.log(`[Gemini] Image fetched (${output.byteLength} bytes)`);
    return { data: Buffer.from(output).toString("base64"), mimeType: "image/jpeg" };
  } catch (error) {
    console.error(`[Gemini] Failed to fetch image from ${cleanUrl}:`, error);
    return null;
  }
}
...
export async function analyzeTrafficCameraImage(
  ...
  console.log(`[Gemini] Analyzing camera: ${locationName}`);
  const imageData = await fetchImageAsBase64(imageUrl);
  if (!imageData) throw new Error("Failed to fetch camera image for analysis");
...
- Be accurate and conservative. Only flag real hazards.`;
  ...
  const validTypes: HazardType[] = ["Flood", "Fire", "Crash", "Stall", "Debris", "Construction", "Weather", "Clear"];
  const parsedType = validTypes.includes(parsed.type) ? parsed.type : "Clear";

  const result: GeminiHazardAnalysis = {
  ...
  console.log(`[Gemini] Camera analysis complete: ${result.type} (severity: ${result.severity}, confidence: ${result.confidence})`);
  return result;
}
...
export async function generateRouteRecommendation(
  ...
  console.log(`[Gemini] Generating route recommendation (${routes.length} routes, ${hazards.length} hazards, eco: ${preferEco})`);
  ...
  const prompt = `You are TrafficRT, an intelligent commute advisor for Atlanta, Georgia. Based on the following real-time data, recommend the best route. Factor in hazard avoidance, air quality, and carbon emissions.
...
Respond ONLY with a valid JSON object (no markdown fences, no extra text):
{
  ...
  const result: GeminiRecommendation = {
    ...
    reasoning: String(parsed.reasoning || "Unable to generate detailed recommendation."),
    healthAdvisory: parsed.healthAdvisory || undefined,
    safetyScore: Math.min(100, Math.max(0, Number(parsed.safetyScore) || 50)),
    ecoScore: Math.min(100, Math.max(0, Number(parsed.ecoScore) || 50)),
  };

  console.log(`[Gemini] Recommendation: ${result.recommendedRouteId} (safety: ${result.safetyScore}, eco: ${result.ecoScore})`);
  return result;
}
...
export async function verifyReportImage(
  ...
  console.log(`[Gemini] Verifying report image (type: ${reportType})`);
  ...
  const prompt = `You are verifying a community infrastructure report. The user reported a "${reportTypeLabels[reportType]}". Analyze this image and determine if the reported issue is actually present.
...
Be fair but accurate. If the image clearly shows the reported issue, verify it. If the image is unclear or doesn't show the issue, mark as not verified.`;
  ...
  const result: GeminiScoutVerification = {
  ...
  console.log(`[Gemini] Report verification: ${result.verified ? "VERIFIED" : "NOT VERIFIED"} (confidence: ${result.confidence})`);
  return result;
}
