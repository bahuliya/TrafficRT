...
import { generateRouteRecommendation, isGeminiConfigured } from "../gemini";
import {
  decodePolyline,
  findNearbyHazards,
...
import { getMockWeather, getMockAirQuality, getMockRoutes } from "../mockData";
...
async function fetchGoogleDirections(
  origin: string,
  destination: string,
  options: { avoidHighways?: boolean; waypoints?: LatLng[] } = {}
): Promise<GoogleDirectionsResponse> {
  if (process.env.DEMO_MODE === "true") throw new Error("Demo mode enabled");
  ...
  if (options.waypoints?.length) {
    params.set("waypoints", options.waypoints.map((wp) => `via:${wp.lat},${wp.lng}`).join("|"));
...
function parseGoogleRoutes(
  ...
  return data.routes.map((route, index) => {
    ...
    if (isHazardAvoiding && isEcoFriendly) routeName = `Safe Eco via ${route.summary || "local roads"}`;
    else if (isHazardAvoiding) routeName = `Safe Route via ${route.summary || "alternate roads"}`;
    else if (isEcoFriendly) routeName = `Eco-Friendly via ${route.summary || "local roads"}`;
    else routeName = `Fastest via ${route.summary || "highway"}`;
...
async function fetchWeatherData(
  ...
  if (!data.main || !data.weather) throw new Error("OpenWeatherMap returned invalid data");
...
async function fetchAirQualityData(
  ...
  if (!apiKey) throw new Error("AIRNOW_API_KEY is not configured");
  ...
  if (!data?.length) return { airQualityIndex: 50, airQualityDescription: "Data temporarily unavailable" };

  const primary = data.reduce((max, obs) => (obs.AQI > max.AQI ? obs : max));
  ...
  const aqiDescriptions: Record<string, string> = {
    ...
    "Unhealthy for Sensitive Groups": "Unhealthy for Sensitive Groups - May affect sensitive individuals",
  ...
  return {
    ...
    airQualityDescription: aqiDescriptions[primary.Category.Name] ?? `${primary.Category.Name} (AQI: ${primary.AQI})`,
  };
}
...
async function fetchAllHazards(): Promise<HazardPoint[]> {
  ...
  try {
    const res = await fetch(`${backendUrl}/api/cameras/hazards/active`);
    if (res.ok) {
      const json = (await res.json()) as { data: Array<{ camId: string; locationName: string; lat: number; lng: number; currentStatus: { type: string; severity: number; geminiExplanation: string } }> };
  ...
  try {
    ...
    if (res.ok) {
      const json = (await res.json()) as { data: Array<{ reportId: string; type: string; description?: string; lat: number; lng: number; status: string }> };
      const reportSeverity: Record<string, number> = {
        flooding: 7, obstruction: 6, pothole: 5, blocked_bike_lane: 4, broken_charger: 3, other: 4,
      };
...
function generateFallbackRecommendation(
  ...
  if (!defaultRoute) return { recommendedRouteId: "unknown", reasoning: "No routes available.", safetyScore: 0, ecoScore: 0 };
  ...
  if (activeHazards > 0 && safeRoute) {
    return {
      ...
      safetyScore: 88, ecoScore: safeRoute.isEcoFriendly ? 85 : 50,
    };
  }
  ...
  return {
    ...
    safetyScore: 85, ecoScore: 45,
  };
}

routesRouter.post("/plan", zValidator("json", RouteRequestSchema), async (c) => {
  ...
  try {
    ...
    if (fastDirections.status === "fulfilled") {
      const r = parseGoogleRoutes(fastDirections.value, false, false);
      if (r[0]) routes.push(r[0]);
    }

    const fastestDistanceKm = routes[0]?.distanceKm;

    if (ecoDirections.status === "fulfilled") {
      const r = parseGoogleRoutes(ecoDirections.value, true, false, fastestDistanceKm);
      if (r[0]) routes.push(r[0]);
    }
    ...
    const hazards: HazardPoint[] = allHazards.status === "fulfilled" ? allHazards.value : [];
    ...
    if (request.avoidHazards && hazards.length > 0) {
      for (const route of [...routes]) {
        ...
        try {
          const waypoints = generateAvoidanceWaypoints(decodePolyline(route.polyline), nearby).slice(0, 5);
          if (!waypoints.length) continue;

          const avoidDirections = await fetchGoogleDirections(request.origin, request.destination, {
            avoidHighways: route.isEcoFriendly,
            waypoints,
          });

          const avoidRoutes = parseGoogleRoutes(avoidDirections, route.isEcoFriendly, true, fastestDistanceKm);
          if (!avoidRoutes[0]) continue;

          const avoidNearby = findNearbyHazards(decodePolyline(avoidRoutes[0].polyline), hazards, HAZARD_RADIUS_METERS);
          avoidRoutes[0].nearbyHazards = avoidNearby;
          avoidRoutes[0].hazardExposureScore = calculateHazardExposureScore(avoidNearby);

          if (avoidRoutes[0].hazardExposureScore < (route.hazardExposureScore ?? 100)) {
    ...
    const weather = weatherResult.status === "fulfilled" ? weatherResult.value : getMockWeather();
    const aqi = aqiResult.status === "fulfilled" ? aqiResult.value : getMockAirQuality();
    ...
    const hazardsOnRoute: HazardPoint[] = [];
    for (const route of routes) {
      for (const h of route.nearbyHazards ?? []) {
        if (!seenIds.has(h.id)) {
          seenIds.add(h.id);
          hazardsOnRoute.push({ id: h.id, lat: h.lat, lng: h.lng, type: h.type, severity: h.severity, description: h.description, source: h.source });
        }
      }
    }
    ...
    if (isGeminiConfigured()) {
      try {
        recommendation = await generateRouteRecommendation(routes, environmental, hazards, request.preferEco ?? false);
      } catch {
        recommendation = generateFallbackRecommendation(routes, environmental, hazards, request.preferEco ?? false);
      }
    } else {
      recommendation = generateFallbackRecommendation(routes, environmental, hazards, request.preferEco ?? false);
    }

    return c.json({ data: { routes, environmental, hazardsOnRoute, recommendation } as RouteResponse });
  } catch (error) {
    console.error("[Routes] Route planning error:", error);
    return c.json(
      { error: { message: error instanceof Error ? error.message : "Failed to plan route", code: "ROUTE_PLANNING_FAILED" } },
      500
    );
  }
});
